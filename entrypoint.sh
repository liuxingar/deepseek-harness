...
# /app/.dsh（DSH_HOME）、/workspace 和 /data（对话数据卷）的宿主目录
# 在 bind mount 后会遮蔽镜像内 chown 的属主设置——NAS 上目录通常归 root
# 所有，导致 dsh 用户无法写入，启动即报：
...
for dir in /app/.dsh /workspace /data; do
  [ -d "$dir" ] || continue
